...
const request: SarvamAI.CreateDubbingJobRequest = {
    ...
};
```
...
```typescript
import { DubbingClient } from 'sarvamai/dubbing';

const client = new DubbingClient({...});
```
...
```typescript
const { data, rawResponse } = await client.dubbing.create(...).withRawResponse();
